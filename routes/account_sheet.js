...
app.post('/queryUser', async function (req, res) {
    ...
    for(var i=data.length-1;i>=0;i--){
        if(data[i].channel=='POINT' && data[i].type==null || data[i].type==null){
...
app.post('/queryUser/sort', async function (req, res) {
	...
	let data = await query(`SELECT `+sort_select+` FROM washing_machine.account_tx `+select_time+select_shop+` union all SELECT  `+sort_select+` FROM (`+select_tx+select_time+select_shop+`)as tx_db`);
	let sort_data=[];
	let notrepeat=[];
